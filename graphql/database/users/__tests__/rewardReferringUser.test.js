...
describe('rewardReferringUser', () => {
  ...
  it('does not give both the referring user and referred user User Impact if they are part of a squad', async () => {
    const userContext = getMockUserContext()
    const userInfo = getMockUserInfo()
    const userId = userInfo.id
    const mockUser = getMockUserInstance(
      Object.assign({ currentMissionId: '123456789' }, userInfo)
    )
    const referringUserId = 'referring-user-id-123'
    ...
    const updateMethod = jest.spyOn(UserImpactModel, 'update')
    await rewardReferringUser(userContext, userId, '123456789')
