...
describe('Background image picker component', () => {
  it('renders without error', () => {
    const BackgroundImagePicker = require('js/components/Background/BackgroundImagePickerComponent')
      .default
    shallow(<BackgroundImagePicker {...mockProps} />)
  })
  ...
  it('calls onBackgroundImageSelection on mount', () => {
    const BackgroundImagePicker = require('js/components/Background/BackgroundImagePickerComponent')
      .default
    shallow(<BackgroundImagePicker {...mockProps} />)
    expect(mockOnBackgroundImageSelection).toHaveBeenCalledWith(
      mockProps.app.backgroundImages.edges[0].node
    )
  })
  ...
  it('visually selects the current image', () => {
    ...
    const selectedImgTile = wrapper
      .find(GridTile)
      .filterWhere(node => node.key() === 'xyz123')
    expect(selectedImgTile.children().find(CheckCircleIcon).length).toBe(1)

    // Another image should not be selected.
    const otherImgTile = wrapper
      .find(GridTile)
      .filterWhere(node => node.key() === 'abcdef')
    expect(otherImgTile.children().find(CheckCircleIcon).length).toBe(0)
    expect(otherImgTile.children().find(RadioButtonUncheckedIcon).length).toBe(
      1
    )
  })
  ...
  it('uses the first image in onBackgroundImageSelection callback on mount if the user does not have an image defined', () => {
    ...
    shallow(<BackgroundImagePicker {...customMockProps} />)
    expect(mockOnBackgroundImageSelection).toHaveBeenCalledWith(
      mockProps.app.backgroundImages.edges[0].node
    )
  })
