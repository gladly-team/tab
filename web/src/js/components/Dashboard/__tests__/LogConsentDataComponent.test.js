...
describe('LogConsentDataComponent', function() {
  it('renders without error and does not have any DOM elements', () => {
    const LogConsentDataComponent = require('js/components/Dashboard/LogConsentDataComponent')
      .default
  ...
  it('logs the consent data when the CMP calls the callback', async () => {
    ...
    const LogUserDataConsentMutation = require('js/mutations/LogUserDataConsentMutation')
      .default
    expect(LogUserDataConsentMutation).toHaveBeenCalledTimes(1)
    expect(LogUserDataConsentMutation.mock.calls[0][0]).toEqual({})
    expect(LogUserDataConsentMutation.mock.calls[0][1]).toEqual(
      'abcdefghijklmno'
    )
    expect(LogUserDataConsentMutation.mock.calls[0][2]).toEqual(
      'some-consent-string'
    )
    ...
    const __runOnCompleted = require('js/mutations/LogUserDataConsentMutation')
      .__runOnCompleted
    __runOnCompleted()
