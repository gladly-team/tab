...
const MuiShallowWithContext = node =>
  shallow(node, {
    context: { appTheme },
...
describe('Sticker campaign component', function() {
  it('renders without error', function() {
    const StickerCampaignComponent = require('js/components/Campaign/StickerCampaignComponent')
      .default
    shallow(<StickerCampaignComponent user={mockUserData} />)
  })
  ...
  it('does not show the success button when there are fewer than 2 active recruits', function() {
    ...
    expect(
      wrapper.find('[data-test-id="sticker-campaign-still-working"]').length
    ).toBe(1)
    expect(
      wrapper.find('[data-test-id="sticker-campaign-success"]').length
    ).toBe(0)
  })
