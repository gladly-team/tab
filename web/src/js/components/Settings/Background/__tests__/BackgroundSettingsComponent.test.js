...
describe('Background settings component', () => {
  ...
  it('renders all of the expected background option radio buttons', () => {
    ...
    backgroundOptions.forEach(backgroundOption => {
      expect(
        wrapper
          .find(RadioButton)
          .filterWhere(node => node.prop('value') === backgroundOption).length
      ).toBe(1)
    })
  ...
  it('selects the correct background option depending on initial background settings option prop', () => {
    ...
    const wrapper = shallow(<BackgroundSettings {...customMockProps} />)

    // Make sure the color option is selected.
    expect(
      wrapper.find(RadioButtonGroup).prop('valueSelected') ===
        USER_BACKGROUND_OPTION_COLOR
    )
  ...
  it('renders new selection components depending on which background option is selected', () => {
    ...
    wrapper.find(RadioButtonGroup).prop('onChange')(
      {},
      USER_BACKGROUND_OPTION_COLOR
    )
    wrapper.update()
  ...
  it('saves settings when the user selects a new photo', () => {
    ...
    wrapper.find(BackgroundImagePicker).prop('onBackgroundImageSelection')(
      mockImg
    )
  ...
  it('saves settings when the user selects a new color', () => {
    ...
    wrapper.find(BackgroundColorPicker).prop('onBackgroundColorSelection')(
      mockColor
    )
  ...
  it('saves settings when the user selects a new custom photo', () => {
    ...
    wrapper.find(BackgroundCustomImagePicker).prop('onCustomImageSelection')(
      mockCustomImgURL
    )

    const mutationMockCall =
      SetBackgroundCustomImageMutation.commit.mock.calls[0]
