...
describe('localBkgStorageMgr', () => {
  it('loads the background option', () => {
    ...
    localStorageMgr.setItem(
      'tab.user.background.imageURL',
      'https://static.example.com/my-img.png'
    )
    localStorageMgr.setItem(
      'tab.user.background.customImage',
  ...
  it('loads the background image URL', () => {
    ...
    expect(getUserBackgroundImageURL()).toEqual(
      'https://static.example.com/my-img.png'
    )
  })
  ...
  it('loads the background custom image', () => {
    ...
    expect(getUserBackgroundCustomImage()).toEqual(
      'https://static.foo.com/some-img.png'
    )
  })

  it('writes to local storage as expected', () => {
    const setBackgroundSettings = require('js/utils/local-bkg-settings')
      .setBackgroundSettings
    setBackgroundSettings(
      'color',
      'https://static.foo.com/blep.png',
      '#000000',
      'https://static.example.com/my-img.png'
    )
  ...
  it('messages the extension parent frame when saving new settings', () => {
    jest.mock('js/utils/extension-messenger')
    const postBackgroundSettings = require('js/utils/extension-messenger')
